...
//Operador For Of Loop
//Permite iterar objetos como arrays, de manera mucho mas sencilla
console.log('For Of');
let animalex = ['Chanchito','Perro','Gato'];
//for (declaracion variableaIterar of ObjetoIterar)
for (let animal of animalex){
    console.log(animal);
}

//Operador For In Loop
//Sirve para iterar las propiedades del objeto.
console.log('For In Original');
let usuario = {
    id: 1,
...
console.log('El valor de las propiedades en orden es ');
for(let propiedad in usuario){
    console.log(usuario[propiedad]);
}

//Permite iterar sobre las posiciones de los objetos.
console.log('For In para numeros de posiciones');
for(let animal in animalex){
    console.log(animal);
}

//Continue y Break
//Break : Corta el ciclo y sale de la iteracion
//Continue : Permite saltarse la iteracion actual y continuar con la siguiente
//Ejemplo Break : Cortamos el ciclo al encontrar Ivan
let arrayPalabras = ['Hola', 'Mundo', 'Soy', 'Ivan','Y','Tengo','32','Años'];
let namex = null;
for(let palabra of arrayPalabras){
    if(palabra.toLowerCase() == 'ivan'){
        namex = palabra;
        break;
    }
}
console.log('Se encontro el nombre de '+namex);

//Ejemplo Continue
let posicion = 0 ;
for(let palabra in arrayPalabras){
    if(arrayPalabras[palabra].toLowerCase() == 'ivan'){
        posicion = palabra;
        continue
    }
}
console.log("Se encontro Ivan en la posicion "+posicion+" del array");

//Ejemplo de uso de CASE
let accion = 'Borrar';
switch (accion){
    case ('Pedir'):
        console.log("Es la accion de pedir");
        break;
    case ('Asignar'):
        console.log("Es la accion de asignar");
        break;
    case ('Cancelar'):
        console.log("Es la accion de cancelar")
        break;
    default:
